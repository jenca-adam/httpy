...
[REFERENCE](httpy.html#submodules)
...
The `Dir` class allows you to store httpy\'s data (cache and cookies) on
the path of your choice. By default, the data is stored in
`~/.cache/httpy`. If you don\'t want to store the data, use the
`enable_cookies` or `enable_cache` argument of `request`. .. code-block:
...
`Session` class. It offers more control over connection closure than the
standard `request`
...
(i.e. with `asyncio.gather`), use the `initiate_http2_connection` method
of `Session`:
...
`Session` and `Dir` and everything with a `request()` method has an
`async_request()` equivalent.
...
-   `Request.socket` - the underlying connection (either `socket.socket`
    or `httpy.http2.connection.HTTP2Connection`)
...
2.  If a charset was not specified, but `chardet` is available, try to
    detect the charset (Note that this still returns `None` if `chardet`
    fails)
...
Do not try to access `Response.string`, if `Response.charset` is `None`,
unless you are absolutely sure the response data is decodable by the
